...
for run in range(run_nr):
    ...
    for ind_segment, segment in enumerate(segments):
        for ind_ch, channel in enumerate(all_channels):
            ...
            train_data = SplitData(doas_train, [channel], ['deep', 'medium', 'light'], [segment], ['all'])
            test_data = SplitData(doas_test, [channel], ['deep', 'medium', 'light'], [segment], ['all'])
            ...
            accuracies[ind_segment][ind_ch].append(acc_test)
            f1scores[ind_segment][ind_ch].append(f1sc_test)
