...
encoding = EncodingCheckBursts('./../../data_to_be_saved/m013/32alphabet3_classic_m013.txt')

#######################################################  L2D3 remove burst ##########################################
print(
    '################################################ L2D3 remove burst quality ################################################################')
csv_file = "svm_L2D3_M013_remove_burst_quality.csv"
csv_results = "svm_L2D3_M013_remove_burst_quality_avr.csv"

output_name_1 = "classification_svm_L2D3_M013_remove_burst_quality.txt"
...
#######################################################  L2D6 remove_burst_quality ##########################################
print(
    '################################################ L2D6 quality ################################################################')
csv_file_2 = "svm_L2D6_M013_remove_burst_quality.csv"
csv_results_2 = "svm_L2D6_M013_remove_burst_quality_avr.csv"

output_name_2 = "classification_svm_L2D6_M013_remove_burst_quality.txt"
...
######################################################  D3L4 remove_burst_qualityv ##########################################
print(
    '################################################ D3L4 quality ################################################################')
csv_file_3 = "svm_D3L4_M013_remove_burst_quality.csv"
csv_results_3 = "svm_D3L4_M013_remove_burst_quality_avr.csv"

output_name_3 = "classification_svm_D3L4_M013_remove_burst_quality.txt"
output_file_3 = open(output_name_3, 'w')

output_file_3.write("Classify bt 2 levels, remove_burst, eliminate and marking bursts and Add a QUALITY feature\n")
output_file_3.write("DEEP3 LIGHT4   stim_spon concatenate A matrix \n")
output_file_3.write("obtain_A_features_from_doa_check_bursts 80% for train \n")
...
#######################################################  L4D6 remove_burst_qualityv ##########################################
print(
    '################################################ L4D6 quality ################################################################')
csv_file_4 = "svm_L4D6_M013_remove_burst_quality.csv"
csv_results_4 = "svm_L4D6_M013_remove_burst_quality_avr.csv"

output_name_4 = "classification_svm_L4D6_M013_remove_burst_quality.txt"
...
######################################################  L4M5D6 remove burst_qualitys ##########################################
print(
    '################################################ L4M5D6 remove burst_qualitys ################################################################')
csv_file_5 = "svm_L4M5D6_M013_remove_burst_quality.csv"
csv_results_5 = "svm_L4M5D6_M013_remove_burst_quality_avr.csv"

output_name_5 = "classification_svm_L4M5D6_M013_remove_burst_quality.txt"
output_file_5 = open(output_name_5, 'w')

output_file_5.write("Classify bt 3 levels, remove_burst, eliminate and marking bursts and Add a QUALITY feature\n")
output_file_5.write("LIGHT4  MEDIUM5 DEEP6  stim_spon concatenate A matrix \n")
output_file_5.write("train_test_doa_remake_balanced 80% for train \n")

# data frame that keeps all runs for all channels, that will be added to .csv file
df_all_5 = DataFrame(columns=column_names_all)
df_all_5.to_csv(csv_file_5, mode='a', header=True)
...
initialization = InitDataSetWithBurstsFlags(current_directory=data_dir, subject_directory="m013",
                                            ...
                                            levels=levels)
doas = initialization.get_dataset_as_doas()
print(f'initialized with {doas[0].level}  {doas[1].level} {doas[2].level}')

mark_bursts_regions(doas, subject='m013')

remove_bursted_trials_when_segment(doas)

accuracies_5 = [[] for i in range(len(all_channels))]
f1scores_5 = [[] for i in range(len(all_channels))]
...
for run in range(run_nr):
    ...
    for chn_ind, channel in enumerate(all_channels):
        print("start running " + str(run) + " for channel " + str(channel))
        output_file_5.write(f'####### run {run} channel {channel} #########\n')

        X_train, y_train = obtain_A_features_from_doa_check_bursts_quality(doas_train, channel, encoding_burst)
        x_test, y_test = obtain_A_features_from_doa_check_bursts_quality(doas_test, channel, encoding_burst)
        ...
        acc_train = report_train['accuracy']
        print(f'predict on train  acc {acc_train}')
        output_file_5.write(f'predict train acc {acc_train}\n')
        ...
        output_file_5.write(classification_report(y_test, predictions))
        np.savetxt(output_file_5, np.array(confusion_matrix(y_test, predictions)), fmt="%s", newline=' ')
        output_file_5.write('\n')

        acc = report['accuracy']
        f1sc = report['weighted avg']['f1-score']
        accuracies_5[chn_ind].append(acc)
        f1scores_5[chn_ind].append(f1sc)

        # calculate and write the mean  and std_dev of the average & f1-score
        df_all_5 = df_all_5.append(
            {'channel': channel, 'segment': segment, 'accuracy': acc, 'f1-score': f1sc}, ignore_index=True)

        # print('debug')
    df_all_5.to_csv(csv_file_5, mode='a', header=False)
    df_all_5 = df_all_5.iloc[0:0]

output_file_5.close()

# data frame that keeps avr and std of the runs
df_results_5 = DataFrame(columns=columns_results_avr)
df_results_5.to_csv(csv_results_5, mode='a', header=True)

for ind_ch, channel in enumerate(all_channels):
    acc_avr = np.mean(np.array(accuracies_5[ind_ch]))
    acc_std = np.std(np.array(accuracies_5[ind_ch]))

    f1_avr = np.mean(np.array(f1scores_5[ind_ch]))
    f1_std = np.std(np.array(f1scores_5[ind_ch]))
    df_results_5 = df_results_5.append({'channel': channel, 'segment': segment, 'acc avr': acc_avr,
                                        'acc std_dev': acc_std, 'f1-sc avr': f1_avr, 'f1-sc std_dev': f1_std},
                                       ignore_index=True)

df_results_5.to_csv(csv_results_5, mode='a', header=False)
