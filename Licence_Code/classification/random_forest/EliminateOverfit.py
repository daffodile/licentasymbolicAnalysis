...
for run in range(run_nr):
    ...
    train_data = ExtractData(doas_train, [channel], ['light', 'medium', 'deep'], [segment], [180])
    test_data = ExtractData(doas_test, [channel], ['light', 'medium', 'deep'], [segment], [180])

    X_train, y_train = obtain_TESPAR_A_FFT_features(train_data, encoding)
    x_test, y_test = obtain_TESPAR_A_FFT_features(test_data, encoding)
    ...
    # # model.fit(X_train, y_train)
    # sfm = SelectFromModel(model, max_features=10)
    # # Train the selector
