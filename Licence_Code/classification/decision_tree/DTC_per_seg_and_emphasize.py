...
from utils.DataSpliting import train_test_doa_remake_balanced, \
    obtain_A_features_from_doa_emphasize_stimulus, obtain_features_labels
...
levels = ['light1', 'deep2']
print(F'FIRST  PER SEGMENT INDIVIDUALY   {levels}')

csv_file_1 = "dtc_per_seg_L1D2_M014_highpass10.csv"
csv_results_1 = "dtc_per_seg_L1D2_M014_highpass10_avr.csv"

output_name = "classification_dtc_per_seg_L1D2_M014_highpass10.txt"
output_file = open(output_name, 'w')

output_file.write("Classify 2 levels, filtered with highpass10 without marking bursts\n")
output_file.write("LIGHT1 DEEP2 a single segment A matrix \n")
...
for run in range(run_nr):
    ...
    for ind_segment, segment in enumerate(segments):
        for ind_channel, channel in enumerate(all_channels):
            ...
            train_data = ExtractData(doas_train, [all_channels[ind_channel]], levels, [segment],
                                     ['all'])
            test_data = ExtractData(doas_test, [all_channels[ind_channel]], levels, [segment],
                                    ['all'])
            ...
            accuracies[ind_segment][ind_channel].append(acc)
            f1scores[ind_segment][ind_channel].append(f1sc)
...
csv_file_2 = "dtc_ephasize_stim_L1D2_M014_highpass10.csv"
csv_results_2 = "dtc_ephasize_stim_L1D2_M014_highpass10_avr.csv"

output_name = "classification_dtc_ephasize_stim_L1D2_m014_highpass10_avr.txt"
output_file = open(output_name, 'w')

output_file.write("Classify 2 levels,filtered with highpass10 without marking bursts\n")
output_file.write("LIGHT1  DEEP2 emphasize stimulus =  A matrix \n")
output_file.write("train_test_doa_remake_balanced 80% for train \n")
...
for run in range(run_nr):
    ...
    for chn_ind, channel in enumerate(all_channels):
        ...
        X_train, y_train = obtain_A_features_from_doa_emphasize_stimulus(doas_train, channel, encoding,selected_symbols=32)
        x_test, y_test = obtain_A_features_from_doa_emphasize_stimulus(doas_test, channel, encoding,selected_symbols=32)
