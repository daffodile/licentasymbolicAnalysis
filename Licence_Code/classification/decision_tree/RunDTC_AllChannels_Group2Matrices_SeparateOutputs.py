...
from utils.DataSpliting import train_test_doa_remake_balanced, obtain_features_labels, obtain_features_labels_quality, \
    obtain_features_labels_log
...
csv_file = "dtc_D2L4_30_32_alph3_log_umarkA_bursts_TA_SPST_classic16.csv"
csv_results = "dtc_D2L4_30_32_alph3_log_umarkA_bursts_TA_SPST_classic16_avg.csv"

classification_reports = "classification_reports_dtc_D2L4_classic16_bursts_log.txt"
output_classification_reports = open(classification_reports, 'w')

confusion_file = 'confusion_matrix_dtc_D2L4_classic16_bursts_log.txt'
output_confusion_matrix = open(confusion_file, 'w')

output_classification_reports.write(
    "Classify deep2 and light4 levels: classic and without marking bursts but with loh\n")
...
# encoding = Encoding('./../../data_to_be_saved/m016_classic_alphabet_3.txt')
# encoding = Encoding('./../../data_to_be_saved/m016_highpass_alphabet_3.txt')
...
for run in range(run_nr):
    ...
    for ind_channel, channel in enumerate(all_channels):
        ...
        train_data = ExtractData(doas_train, [all_channels[ind_channel]], ['deep2', 'light4'], segments,
                                 ['all'])
        test_data = ExtractData(doas_test, [all_channels[ind_channel]], ['deep2', 'light4'], segments,
                                ['all'])
        ...
        df_all = df_all.append(
            {'channel': channel, 'segment': segments[0] + '+' + segments[1], 'accuracy': acc, 'f1-score': f1sc},
            ignore_index=True)
