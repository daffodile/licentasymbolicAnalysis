'''
    RUN FFT + A TESPAR 12 symbols here
'''
...
from feature_extraction.FFT.FFTFeatures import obtain_concatenate_segments_FFT_TESPAR_A, \
    obtain_concatenate_segments_FFT_TESPAR_A_remove_burst
...
######################################################  L1D2 classic_FFT and TESPAR ##########################################
print(
    '############################################### L1D2 classic_FFT and TESPAR A  ################################################################')
csv_file_4 = "svm_L1D2_M014_classic_FFT_TESPAR_A.csv"
csv_results_4 = "svm_L1D2_M014_classic_FFT_TESPAR_A_avr.csv"

output_name_4 = "classification_svm_L1D2_M014_classic_FFT_TESPAR_A.txt"
output_file_4 = open(output_name_4, 'w')

output_file_4.write("Classify bt 2 levels classic, no mark \n")
...
#######################################################  L1D2M3 remove burst ##########################################
print(
    '################################################ L1D2M3 classic fft tesapr a ################################################################')
csv_file = "svm_L1D2M3_M014_classic_FFT_TESPAR_A.csv"
csv_results = "svm_L1D2M3_M014_classic_FFT_TESPAR_A_avr.csv"

output_name_1 = "classification_svm_L1D2M3_M014_classic_FFT_TESPAR_A.txt"
...
#######################################################  L1D2 remove_burst_FFT and TESPAR ##########################################
print(
    '############################################### L1D2 remove_burst FFT and TESPAR A  ################################################################')
xsv_file = "svm_L1D2_M014_remove_burst_FFT_TESPAR_A.csv"
xsv_results = "svm_L1D2_M014_remove_burst_FFT_TESPAR_A_avr.csv"

output_name = "classification_svm_L1D2_M014_remove_burst_FFT_TESPAR_A.txt"
output_file_x = open(output_name, 'w')

output_file_x.write("Classify bt 2 levels remove_burst and mark \n")
output_file_x.write("LIGHT1 DEEP2 spon_Stim concat FFT descriptors and TESPAR A \n")
output_file_x.write("train_test_doa_balanced_on_train_on_test 80% for train \n")

# data frame that keeps all runs for all channels, that will be added to .csv file
df_all_x = DataFrame(columns=column_names_all)
df_all_x.to_csv(xsv_file, mode='a', header=True)
...
accuracies_x = [[] for i in range(len(all_channels))]
f1scores_x = [[] for i in range(len(all_channels))]
...
for run in range(run_nr):
    ...
    for chn_ind, channel in enumerate(all_channels):
        print("start running " + str(run) + " for channel " + str(channel))
        output_file_x.write(f'####### run {run} channel {channel} #########\n')

        train_data = ExtractData(doas_train, [channel], levels, segments, ['all'])
        test_data = ExtractData(doas_test, [channel], levels, segments, ['all'])

        X_train, y_train = obtain_concatenate_segments_FFT_TESPAR_A_remove_burst(train_data, channel, encoding)
        x_test, y_test = obtain_concatenate_segments_FFT_TESPAR_A_remove_burst(test_data, channel, encoding)
        ...
        acc_train = report_train['accuracy']
        print(f'predict on train  acc {acc_train}')
        output_file_x.write(f'predict train acc {acc_train}\n')
        ...
        output_file_x.write(classification_report(y_test, predictions))
        np.savetxt(output_file_x, np.array(confusion_matrix(y_test, predictions)), fmt="%s", newline=' ')
        output_file_x.write('\n')

        acc = report['accuracy']
        f1sc = report['weighted avg']['f1-score']
        accuracies_x[chn_ind].append(acc)
        f1scores_x[chn_ind].append(f1sc)

        # calculate and write the mean  and std_dev of the average & f1-score
        df_all_x = df_all_x.append(
            {'channel': channel, 'segment': segment, 'accuracy': acc, 'f1-score': f1sc}, ignore_index=True)

        # print('debug')
    df_all_x.to_csv(xsv_file, mode='a', header=False)
    df_all_x = df_all_x.iloc[0:0]

output_file_x.close()

# data frame that keeps avr and std of the runs
df_results_x = DataFrame(columns=columns_results_avr)
df_results_x.to_csv(xsv_results, mode='a', header=True)

for ind_ch, channel in enumerate(all_channels):
    acc_avr = np.mean(np.array(accuracies_x[ind_ch]))
    acc_std = np.std(np.array(accuracies_x[ind_ch]))

    f1_avr = np.mean(np.array(f1scores_x[ind_ch]))
    f1_std = np.std(np.array(f1scores_x[ind_ch]))
    df_results_x = df_results_x.append({'channel': channel, 'segment': segment, 'acc avr': acc_avr,
                                        'acc std_dev': acc_std, 'f1-sc avr': f1_avr, 'f1-sc std_dev': f1_std},
                                       ignore_index=True)

df_results_x.to_csv(xsv_results, mode='a', header=False)


# THIS ONE IS NOT RUN, BT ERROR OCCURED
#######################################################  L1D2M3 remove burst ##########################################
print(
    '################################################ L1D2M3 remove burst FFT tespar A ################################################################')
csv_file = "svm_L1D2M3_M014_remove_burst_FFT_TESPAR_A.csv"
csv_results = "svm_L1D2M3_M014_remove_burst_FFT_TESPAR_A_avr.csv"

output_name_1 = "classification_svm_L1D2M3_M014_remove_burst_FFT_TESPAR_A.txt"
output_file_1 = open(output_name_1, 'w')

output_file_1.write("Classify bt3 levels, remove_burst and mark\n")
output_file_1.write("light1 deep2 medium3  stim_spon FFT and TESPAR A \n")
output_file_1.write("train_test_doa_balanced_on_train_on_test 80% for train \n")
